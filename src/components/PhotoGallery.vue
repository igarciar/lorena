<template>
  <div class="grid">
    <img v-for="(img, i) in images" :src="img" @click="open(i)" class="thumb" />
  </div>
  <div v-if="show" class="full" @click="close">
    <button @click.stop="prev">⟨</button><img :src="current" class="fullimg" /><button
      @click.stop="next"
    >
      ⟩
    </button>
